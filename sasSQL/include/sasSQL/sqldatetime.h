...
public:
	...
	SQLDateTime(tm * t);
	SQLDateTime(unsigned int year, unsigned int months, unsigned int days, unsigned int hours, unsigned int minutes, unsigned int seconds, int msecs = -1, bool daylightSaveTime = false, bool negative = false);
	virtual ~SQLDateTime();
